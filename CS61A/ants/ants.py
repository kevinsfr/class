...
class Place:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            "*** YOUR CODE HERE ***"
            if (insect.name!='Queen') or ((insect.name=='Queen') and (insect.num>1)):
                if insect.container:
                    if insect.ant!=None:
                        self.ant=insect.ant
                else:
...
class Ant(Insect):
    ...
    def can_contain(self,other):
        if self.container:
            return (self.ant==None) and (not other.container)
        else:
            return False
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        if self.num>1:
            ...
            p2=self.place.exit
            while p1!=None:
                if (p1.ant!=None) and (not p1.ant in self.ant_list) and (p1.ant.name!='Queen'):
                    p1.ant.damage*=2
                    self.ant_list+=[p1.ant]
                    if (hasattr(p1.ant,'ant')) and (p1.ant.ant!=None) and (p1.ant.ant.name!='Queen'):
                        p1.ant.ant.damage*=2
                        self.ant_list+=[p1.ant.ant]
                p1=p1.entrance
            while p2!=None:
                if (p2.ant!=None) and (not p2.ant in self.ant_list) and (p2.ant.name!='Queen'):
                    p2.ant.damage*=2
                    self.ant_list+=[p2.ant]
                    if (hasattr(p2.ant,'ant')) and (p2.ant.ant!=None) and (p2.ant.ant.name!='Queen'):
                        p2.ant.ant.damage*=2
                        self.ant_list+=[p2.ant.ant]
                p2=p2.exit
