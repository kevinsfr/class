...
def extract_words(text):
    ...
    while i<l:
        j,x=i,''
        while (j<l) and (text[j] in ascii_letters):
            x=x+text[j]
            j+=1
        if x!='':
            s=s+[x]
...
def analyze_tweet_sentiment(tweet):
    ...
    total,x=0,0
    for w in words:
        s=get_word_sentiment(w)
        if has_sentiment(s):
            total+=sentiment_value(s)
            x+=1
    if x>0:
        return make_sentiment(total/x)
    else:
...
def find_centroid(polygon):
    ...
    """
    "*** YOUR CODE HERE ***"
    i,x,y,total,n=0,0,0,0,len(polygon)-1
    while i<n:
        p1,p2=polygon[i],polygon[i+1]
        a=latitude(p1)*longitude(p2)-latitude(p2)*longitude(p1)
        total+=a
        x+=(latitude(p1)+latitude(p2))*a
        y+=(longitude(p1)+longitude(p2))*a
        i+=1
    area=total/2
    if area!=0:
        return [x/(area*6),y/(area*6),abs(area)]
    else:
