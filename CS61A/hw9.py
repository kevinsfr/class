...
def brack_read(tokens):
    ...
    if n != None:
        ...
        "*** YOUR CODE HERE ***"
        return Pair(brackets[(token,left_to_right[token])],read_tail(tokens,token))
    else:
...
class Mobile:
    ...
    def __init__(self, left, right):
        "*** YOUR CODE HERE ***"
        if not (type(left) is Branch):
            raise TypeError(str(left)+' is not a Branch')
        if not (type(right) is Branch):
...
def interpret_mobile(s):
    ...
    for t in ('Branch', 'Weight', 'Mobile'):
        "*** YOUR CODE HERE ***"
        interpret_mobile(s[0:next_T]+t+s[next_T:])
    return None
...
def merge(s0, s1):
    ...
    x1=s1.first
    if x0==x1:
        return Stream(x0,lambda:merge(s0.rest,s1.rest))
    elif x0>x1:
        return merge(s1,s0)
    else:
        return Stream(x0,lambda:merge(s0.rest,s1))
...
def unique(s):
    ...
    def efilter(s,fn):
        if s==Stream.empty:
            return s
        if fn(s.first):
            return efilter(s.rest,fn)
        else:
            return Stream(s.first, lambda: efilter(s.rest,fn))
    def rest():
        return unique(efilter(s,lambda x: x==s.first))
...
def rle(s, max_run_length=10):
    ...
    k=0
    lst=[]
    while (k<max_run_length) and (s!=Stream.empty):
        lst+=[s.first]
        s=s.rest
        k+=1
    if k<max_run_length:
        return rle(to_stream(lst),k)
    us=unique(to_stream(lst))
    def count(lst,k):
        num=0
        for x in lst:
            if k==x:
    ...
    def rest():
        nonlocal us
        us=us.rest
        if us is Stream.empty:
            return rle(s,max_run_length)
        return Stream(count(lst,us.first),rest)
    return Stream(count(lst,us.first),rest)
