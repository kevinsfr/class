...
def cornersHeuristic(state, problem):
    ...
    while goals and k < 1:
        ...
        closest = goals[0]
        for goal in goals:
            distance = util.manhattanDistance(currentPos, goal)
...
def foodHeuristic(state, problem):
    ...
    for i in range(wd):
        for j in range(ht):
            if foodGrid[i][j]:
                h = max(h, abs(i - position[0]) + abs(j - position[1]))
    return h
