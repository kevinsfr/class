...
def fillObsCPT(bayesNet, gameState):
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            obsVarFactor = bn.Factor([obsVar], [GHOST_HOUSE_VAR, FOOD_HOUSE_VAR], bayesNet.variableDomainsDict())
            for assignment in obsVarFactor.getAllPossibleAssignmentDicts():
                ...
                if housePos == bottomLeftPos:
                    if foodHousePos == BOTTOM_LEFT_VAL:
                    ...
                    elif ghostHousePos == BOTTOM_LEFT_VAL:
                        if assignment[obsVar] == RED_OBS_VAL:
                ...
                if housePos == topLeftPos:
                    if foodHousePos == TOP_LEFT_VAL:
                        if assignment[obsVar] == RED_OBS_VAL:
                            ...
                            obsVarFactor.setProbability(assignment, 0)
                    elif ghostHousePos == TOP_LEFT_VAL:
                        if assignment[obsVar] == RED_OBS_VAL:
                            obsVarFactor.setProbability(assignment, PROB_GHOST_RED)
                        elif assignment[obsVar] == BLUE_OBS_VAL:
                            obsVarFactor.setProbability(assignment, 1 - PROB_GHOST_RED)
                ...
                if housePos == bottomRightPos:
                    if foodHousePos == BOTTOM_RIGHT_VAL:
                        if assignment[obsVar] == RED_OBS_VAL:
                            obsVarFactor.setProbability(assignment, PROB_FOOD_RED)
                        elif assignment[obsVar] == BLUE_OBS_VAL:
                            obsVarFactor.setProbability(assignment, 1 - PROB_FOOD_RED)
                            ...
                            obsVarFactor.setProbability(assignment, 0)
                    elif ghostHousePos == BOTTOM_RIGHT_VAL:
                        if assignment[obsVar] == RED_OBS_VAL:
                            obsVarFactor.setProbability(assignment, PROB_GHOST_RED)
                        elif assignment[obsVar] == BLUE_OBS_VAL:
                            obsVarFactor.setProbability(assignment, 1 - PROB_GHOST_RED)
                ...
                if housePos == topRightPos:
                    if foodHousePos == TOP_RIGHT_VAL:
                        if assignment[obsVar] == RED_OBS_VAL:
                            ...
                            obsVarFactor.setProbability(assignment, 0)
                    elif ghostHousePos == TOP_RIGHT_VAL:
                        if assignment[obsVar] == RED_OBS_VAL:
                            obsVarFactor.setProbability(assignment, PROB_GHOST_RED)
                        elif assignment[obsVar] == BLUE_OBS_VAL:
                            obsVarFactor.setProbability(assignment, 1 - PROB_GHOST_RED)
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    maximum = float("-inf")
    mostPossible = factor.getAllPossibleAssignmentDicts()[0]
    for assignment in factor.getAllPossibleAssignmentDicts():
        if factor.getProbability(assignment) > maximum:
            maximum = factor.getProbability(assignment)
            mostPossible = assignment
    return mostPossible
