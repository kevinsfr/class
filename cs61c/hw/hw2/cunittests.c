...
void simple_status_test(void){
    ...
    FILE* zxcv = fopen("zxcv.txt", "w");
    fclose(zxcv);
    retval = beargit_add("zxcv.txt");
    CU_ASSERT(0==retval);
    ...
    CU_ASSERT_PTR_NOT_NULL(fgets(line, LINE_SIZE, fstdout));
    CU_ASSERT(!strcmp(line, "Track Files:\n"));

    CU_ASSERT_PTR_NOT_NULL(fgets(line, LINE_SIZE, fstdout));
    CU_ASSERT(!strcmp(line,"\n"));

    CU_ASSERT_PTR_NOT_NULL(fgets(line, LINE_SIZE, fstdout));
    CU_ASSERT(!strcmp(line, "  asdf.txt\n"));

    CU_ASSERT_PTR_NOT_NULL(fgets(line, LINE_SIZE, fstdout));
    CU_ASSERT(!strcmp(line, "  qwer.txt\n"));

    CU_ASSERT_PTR_NOT_NULL(fgets(line, LINE_SIZE, fstdout));
    CU_ASSERT(!strcmp(line, "  zxcv.txt\n"));

    CU_ASSERT_PTR_NOT_NULL(fgets(line, LINE_SIZE, fstdout));
    CU_ASSERT(!strcmp(line,"\n"));

    CU_ASSERT_PTR_NOT_NULL(fgets(line, LINE_SIZE, fstdout));
    CU_ASSERT(!strcmp(line, "3 files total\n"));    
...
void simple_branch_test(void){
    ...
    retval = beargit_checkout("asdf", 1);
    CU_ASSERT(0==retval);

    retval = beargit_checkout("qwer", 1);
    CU_ASSERT(0==retval);
  
    retval = beargit_checkout("zxcv", 1);
    CU_ASSERT(0==retval);

    retval = beargit_checkout("zxcv", 0);
    CU_ASSERT(0==retval);
    ...
    CU_ASSERT_PTR_NOT_NULL(fgets(line, LINE_SIZE, fstdout));
    CU_ASSERT(!strcmp(line,"  asdf\n"));

    CU_ASSERT_PTR_NOT_NULL(fgets(line, LINE_SIZE, fstdout));
    CU_ASSERT(!strcmp(line, "  qwer\n"));

    CU_ASSERT_PTR_NOT_NULL(fgets(line, LINE_SIZE, fstdout));
    CU_ASSERT(!strcmp(line, "* zxcv\n"));
...
int cunittester()
{
   ...
   if (NULL == CU_add_test(pSuite3, "Status output test", simple_status_test))
   {
      CU_cleanup_registry();
      return CU_get_error();
   }
   ...
   if (NULL == CU_add_test(pSuite4, "Branch output test", simple_branch_test))
   {
      CU_cleanup_registry();
      return CU_get_error();
   }
