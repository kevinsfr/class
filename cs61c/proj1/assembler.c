...
int pass_one(FILE* input, FILE* output, SymbolTable* symtbl) {
    ...
    char* str, *name;
    char* args[MAX_ARGS];
    int x, num_line = 0, num_args, f = 0, byte_offset = 0;
    while (fgets(buf, sizeof(buf), input)){
        name = NULL;
        num_args = 0;
        num_line++;
        skip_comment(buf);
        str = strtok(buf, IGNORE_CHARS);
        if (str != NULL){
            x = add_if_label(num_line, str, byte_offset, symtbl);
            if (x == 0){
                name = str;
            }
            else if (x == 1){
                str = strtok(NULL, IGNORE_CHARS);
                if (str != NULL){
                    name = str;
                }
            }
                ...
                while (str != NULL){
                    ...
                    if (str != NULL){
                        ...
                        if (num_args > MAX_ARGS){
                            raise_extra_arg_error(num_line, str);
                            f = 1;
                            break;
                        }
                    }
                }
                if (num_args <= MAX_ARGS && name != NULL){
                    x = write_pass_one(output, name, args, num_args);
                    if (x == -1){
                        f = 1;
                    }
                    else{
...
int pass_two(FILE *input, FILE* output, SymbolTable* symtbl, SymbolTable* reltbl) {
    ...
    while (fgets(buf, sizeof(buf), input)){
        ...
        if (name != NULL){
            err = translate_inst(output, name, args, num_args, addr, symtbl, reltbl);
        }
        if (err == -1){
            raise_inst_error(num_line, name, args, num_args);
            f = 1;
        }
        addr += 4;
    }
