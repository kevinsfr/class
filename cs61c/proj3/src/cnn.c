...
double func(vol_t* f, int fy, int fx, int fd, vol_t* V, int V_sx, int ox, int oy) {
  return f->w[((f->sx * fy)+fx)*f->depth+fd] * V->w[((V_sx * oy)+ox)*V->depth+fd];
}
          ...
          for(int fy = 0; fy < f->sy; fy++) {
            ...
            for(int fx = 0; fx < f->sx; fx++) {
              ...
              if(oy >= 0 && oy < V_sy && ox >=0 && ox < V_sx) {
                for(int fd=0;fd < f->depth; fd+=4) {
                  a += func(f, fy, fx, fd, V, V_sx, ox, oy);
                  if (fd + 1 < f->depth) {
                    a += func(f, fy, fx, fd + 1, V, V_sx, ox, oy);
                  }
                  if (fd + 2 < f->depth) {
                    a += func(f, fy, fx, fd + 2, V, V_sx, ox, oy);
                  }
                  if (fd + 3 < f->depth) {
                    a += func(f, fy, fx, fd + 3, V, V_sx, ox, oy);
                  }
                }
              }
            }
