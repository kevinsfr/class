...
class NNClassifier(Classifier):
  ...
  def forward(self, data):
    ...
    A3 = self.A3
    b3 = self.b3
    layer1 = data.map(lambda (k, (x, y)): (k, linear_forward(x, A1, b1)))
    layer2 = layer1.map(lambda (k, lfreturn) :(k,lfreturn, ReLU_forward(lfreturn)))
    layer3 = layer2.map(lambda (k, lfreturn, relureturn) : (k, lfreturn, linear_forward(relureturn, A3, b3)))
    return layer3.map(lambda (k, lfreturn, relureturn, lfreturn2) : [lfreturn, relureturn, lfreturn2])
