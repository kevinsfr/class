...
class LearningSwitch(api.Entity):
    ...
    def handle_rx(self, packet, in_port):
        ...
        src = packet.src
        dst = packet.dst
        if self.routing_table.has_key(src):
            if in_port not in self.routing_table[src]:
                self.routing_table[src].append(in_port)
        else:
            self.routing_table[src] = [in_port]
        if self.routing_table.has_key(dst):
            self.send(packet, self.routing_table[dst])
        else:
